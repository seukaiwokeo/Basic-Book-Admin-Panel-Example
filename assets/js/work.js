...
var lid = 0; // son düzenlenmek istenen kitabın id si global
var editBookM; // global kullanım için
...
function getBooks(q) // kitap listesini çekelim
{
	$.ajax({
			...
			url: "api/request.php?a=book_list", // api url
			data: {
				q: q
			}
		})
...
function editBook(id) {
	...
	$.ajax({
			type: "POST", // route dışındaki tüm parametreleri post olarak aldığımız için
			url: "api/request.php?a=book", // api url
			data: {
				q: lid
			}
		})
...
$("#cover").change(function() {
	...
	if (files.length > 0) {
		...
		$.ajax({
			url: "api/request.php?a=upload_cover",
			type: "post",
			data: fd,
			contentType: false,
			processData: false,
...
$("#adding_book").submit(function(event) {
	if (this.checkValidity()) {
		...
		$.ajax({
				type: "POST", // route dışındaki tüm parametreleri post olarak aldığımız için
				url: "api/request.php?a=book_add", // api url
				data: {
					title: title,
					summary: summary,
					cover: cover,
...
$("#saving_book").submit(function(event) {
	if (this.checkValidity()) {
		...
		$.ajax({
				type: "POST", // route dışındaki tüm parametreleri post olarak aldığımız için
				url: "api/request.php?a=book_edit", // api url
				data: {
					id: lid,
					title: title,
					summary: summary,
...
function removeBook() {
	$.ajax({
			type: "POST", // route dışındaki tüm parametreleri post olarak aldığımız için
			url: "api/request.php?a=book_remove", // api url
			data: {
				q: lid
			}
		})
...
$(function() {
	$.fn.datepicker.defaults.format = "yyyy-mm-dd"; // veritabanında varsayılan olarak yyyy-mm-dd şeklinde kabul ettiği için yapıyoruz
